...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 bg-[var(--bg)] text-[var(--ink)]">
      ...
      <div className="w-full max-w-sm card p-6 sm:p-8" style={{ boxShadow: 'var(--shadow-md)' }}>
        {status === 'sent' ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 h-11 bg-[var(--surface-2)] border border-[var(--border)] rounded-lg text-sm text-[var(--ink)] placeholder-[var(--ink-3)] focus:outline-none focus:border-[var(--brand)] focus:ring-2 focus:ring-[var(--brand-soft)]"
                />
              </div>
              <button
                type="submit"
                disabled={status === 'sending'}
                className="flex justify-center items-center gap-2 h-11 rounded-lg font-semibold text-white bg-[var(--brand)] hover:brightness-95 transition-all disabled:opacity-50"
              >
