...
export default function ReportHeader({ domain, url, generatedAt }: { domain: string; url: string; generatedAt: string }) {
  ...
  return (
    <header className="no-print sticky top-0 z-40 bg-[var(--surface)]/90 backdrop-blur-md border-b border-[var(--border)] h-16 sm:h-[68px] flex items-center justify-between px-3 sm:px-6 gap-2">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <h1 className="text-[13px] sm:text-[15px] font-bold text-[var(--ink)] tracking-tight leading-tight truncate">Audit Report — {domain}</h1>
