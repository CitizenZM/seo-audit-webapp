interface Keyword {
  keyword: string;
  intent: string;
  volume: string;
  difficulty: string;
}

export default function KeywordTable({ keywords }: { keywords: Keyword[] }) {
  if (!keywords || keywords.length === 0) return null;

  return (
    <div className="bg-[var(--mid)] border border-[#2a2a4a] rounded-xl overflow-hidden mt-6">
      <div className="p-6 border-b border-[#2a2a4a]">
        <h3 className="text-lg font-bold text-white">Keyword Opportunities</h3>
        <p className="text-sm text-[var(--muted)] mt-1">Identified high and medium intent targets based on content gaps.</p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-[rgba(201,168,76,0.1)] text-[var(--gold)] uppercase tracking-wider text-xs">
            <tr>
              <th className="px-6 py-4">Keyword</th>
              <th className="px-6 py-4">Intent</th>
              <th className="px-6 py-4">Difficulty</th>
              <th className="px-6 py-4">Est. Volume</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#1e1e3a]">
            {keywords.map((kw: Keyword, idx: number) => (
              <tr key={idx} className="hover:bg-[rgba(255,255,255,0.02)] transition-colors border-t border-[#2a2a4a]">
                <td className="p-3 font-semibold text-[#ddd]">{kw.keyword}</td>
                <td className="px-6 py-4 text-[#ddd]">{kw.intent}</td>
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${kw.difficulty?.toLowerCase() === 'high' ? 'bg-[rgba(231,76,60,0.15)] text-[#e74c3c] border-[rgba(231,76,60,0.3)]' : kw.difficulty?.toLowerCase() === 'medium' ? 'bg-[rgba(243,156,18,0.15)] text-[#f39c12] border-[rgba(243,156,18,0.3)]' : 'bg-[rgba(46,204,113,0.15)] text-[#2ecc71] border-[rgba(46,204,113,0.3)]'}`}>
                    {kw.difficulty || 'Low'}
                  </span>
                </td>
                <td className="px-6 py-4 text-[#aaa]">{kw.volume || 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
