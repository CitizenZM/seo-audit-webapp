  ...
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden mt-6">
      <div className="p-6 border-b border-[var(--border)]">
        <h3 className="text-lg font-bold text-[var(--ink)]">Keyword Opportunities</h3>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {keywords.map((kw: Keyword, idx: number) => (
              <tr key={idx} className="hover:bg-[var(--surface-2)] transition-colors border-t border-[var(--border)]">
                <td className="p-3 font-semibold text-[var(--ink-2)]">{kw.keyword}</td>
                <td className="px-6 py-4 text-[var(--ink-2)]">{kw.intent}</td>
                ...
                <td className="px-6 py-4 text-[var(--ink-3)]">{kw.volume || 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
